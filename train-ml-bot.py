...
from bots.inverserdeep import inverserdeep
...
def create_dataset(path, player=rand.Bot(), games=300, phase=1):
...
if options.overwrite or not os.path.isfile(options.dset_path):
    create_dataset(options.dset_path, player=inverserdeep.Bot(), games=5000)
