...
TOTAL_GAMES = 5000
GAMES_PER_BOT = TOTAL_GAMES / 2


def create_dataset(path, player1, player2, games, phase=1):
    ...
    for g in range(games-1):
        ...
        #if g % 3 == 1:  <---------- copy code from previous block and change bot game distribution (line 50)
...
if options.overwrite or not os.path.isfile(options.dset_path):
    create_dataset(options.dset_path, player1=rdeep.Bot(), player2=kbbot.Bot(), games=TOTAL_GAMES)
